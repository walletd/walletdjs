...
export class BitcoinEsploraApiProvider extends Chain<BitcoinEsploraBaseProvider> {
  ...
  public async getBalance(_addresses: AddressType[]): Promise<BigNumber[]> {
    ...
    return [
      utxos.reduce((acc, utxo) => acc.plus(utxo.value), new BigNumber(0)),
    ];
  }
  ...
  public async getUnspentTransactions(
    ...
    return this.provider.getUnspentTransactions(
      _addresses,
    ) as unknown as UTXO[];
  }
  ...
  public async getSpendStatus(utxo: UTXO): Promise<boolean> {
    ...
    try {
      data = await this._httpClient.nodeGet(
        `/tx/${utxo.txid}/outspend/${utxo.vout}`,
      );
      return data.spent;
    } catch (e) {
      const err = e as AxiosError;
  ...
  public async getBalance2(utxos: UTXO[]): Promise<BigNumber[]> {
    ...
    return [
      _utxos.reduce((acc, _utxo) => acc.plus(_utxo.value), new BigNumber(0)),
    ];
  }
}
