...
export class BitcoinWallet2 {
  ...
  async send(
    ...
    for (const unspent of this.unspent) {
      const keyPair = this.addressNew.find(
        (addressNew) => addressNew.address === unspent.address,
      )?.keyPair;
      ...
      if (this.generateAddress(i)?.address! !== undefined) {
        ...
        if (data.chain_stats.tx_count > 0) {
          transactions = await provider.getAddressTransactions(
            this.generateAddress(i)?.address!,
          );
        }
        this.addresses.push(
          new Address(
...
export function buildp2pkh(
  unspents: Array<InputInterface>,
