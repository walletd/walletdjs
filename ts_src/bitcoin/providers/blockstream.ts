...
export class Blockstream implements Provider {
  ...
  getBlocks(height: number): Promise<Array<BlockResult>> {
    return request<Array<BlockstreamBlockResult>>(
      this.url + "/blocks/" + height,
    ).then((data) => {
  ...
  getBlockByHash(hash: string): Promise<BlockResult> {
    return request<BlockstreamBlockResult>(this.url + "/block/" + hash).then(
      (data) => {
        return {
          hash: data.id,
          ver: data.version,
  ...
  getAddressTransactions(address: string): Promise<Array<TransactionResult>> {
    return request<Array<TransactionResult>>(
      this.url + "/address/" + address + "/txs",
    );
  }

  getTransaction(txid: string): Promise<TransactionInterface> {
    return request<TransactionResult>(this.url + "/tx/" + txid).then((data) => {
      return {
        txId: data.txid,
        txHex: "",
        vsize: data.size,
